...
const A = (o: any[] | Float64Array, a: any[] | Float64Array, b: any[] | Float64Array) => {
  for (let i = 0; i < 16; i++) {
    o[i] = a[i] + b[i];
  }
};

const Z = (o: Float64Array | number[], a: Float64Array | number[], b: Float64Array | number[]) => {
  for (let i = 0; i < 16; i++) {
    o[i] = a[i] - b[i];
  }
};

const M = (o: Float64Array | number[], a: any[] | Float64Array, b: any[] | Float64Array) => {
  let v: number,
    c: number,
    t0 = 0,
    t1 = 0,
...
const vn = (x: Uint8Array, xi: any, y: Uint8Array, yi: any, n: number) => {
  let i: number,
    d = 0;
  for (i = 0; i < n; i++) {
    d |= x[xi + i] ^ y[yi + i];
...
const pow2523 = (o: Float64Array, i: Float64Array) => {
  const c = gf();
  let a: number;
  for (a = 0; a < 16; a++) {
    c[a] = i[a];
...
const inv25519 = (o: Float64Array, i: any[] | Float64Array) => {
  const c = gf();
  let a: number;
  for (a = 0; a < 16; a++) {
    c[a] = i[a];
...
const set25519 = (r: { [x: string]: number }, a: Float64Array) => {
  let i: number;
  for (i = 0; i < 16; i++) {
    r[i] = a[i] | 0;
  }
...
const car25519 = (o: Float64Array | number[]) => {
  let i: number,
    v: number,
    c = 1;
  for (i = 0; i < 16; i++) {
...
const sel25519 = (p: Float64Array | number[], q: Float64Array | number[], b: number) => {
  let t: number,
...
const pack25519 = (o: Uint8Array | number[], n: Float64Array) => {
...
const cswap = (p: { [x: string]: any }, q: { [x: string]: any }, b: any) => {
  let i: number;
  for (i = 0; i < 4; i++) {
    sel25519(p[i], q[i], b);
  }
...
const neq25519 = (a: Float64Array, b: Float64Array) => {
...
const par25519 = (a: Float64Array) => {
  const d = new Uint8Array(32);
  pack25519(d, a);
  return d[0] & 1;
};

const unpack25519 = (o: number[], n: Uint8Array) => {
  let i: number;
  for (i = 0; i < 16; i++) {
    o[i] = n[2 * i] + (n[2 * i + 1] << 8);
  }
